...
const createComponents = (headingColor = 'text-gray-900', descriptionColor = 'text-gray-700') => ({
  ...
  block: {
    normal: ({children}: any) => (
      <p className={`text-base lg:text-lg leading-relaxed font-normal ${descriptionColor}`}>
        {children}
      </p>
    ),
    h1: ({children}: any) => (
      <h1 className={`text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black mt-8 mb-6 leading-tight tracking-tight ${headingColor}`}>
        {children}
      </h1>
    ),
    h2: ({children}: any) => (
      <h2 className={`text-3xl sm:text-4xl lg:text-5xl font-bold mt-6 mb-4 leading-tight ${headingColor}`}>
        {children}
      </h2>
    ),
    h3: ({children}: any) => (
      <h3 className={`text-2xl sm:text-3xl lg:text-4xl font-bold mt-4 mb-3 leading-snug ${headingColor}`}>
        {children}
      </h3>
    ),
    h4: ({children}: any) => (
      <h4 className={`text-xl sm:text-2xl lg:text-3xl font-semibold mt-3 mb-2 ${headingColor}`}>
        {children}
      </h4>
    ),
    blockquote: ({children}: any) => (
      <blockquote className={`border-l-4 border-amber-500 pl-4 italic my-4 text-lg ${descriptionColor}`}>
        {children}
      </blockquote>
    ),
  },
...
export default function RichTextRenderer({ 
  ...
  // 👇 Handle if content is customRichText type (has .text property)
  const actualContent = content?.text || content;
  // 👇 Handle if content is array with customRichText type
